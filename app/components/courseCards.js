...
const CourseCard =({courseData}) =>{
  ...
  return (
    <Link className="group flex flex-col font-Poppins rounded-xl bg-dark-blue justify-between transition-all shadow-md shadow-black hover:shadow-lg hover:shadow-black" href={`/courses/${slug}`}>
      <div className="">
        ...
        <p className="bg-bg-blue rounded-full m-auto text-sm w-fit py-px px-4 -translate-y-3 shadow-md shadow-black">{category_name}</p>
      </div>
      <p className="text-left font-medium text-xl capitalize px-4 pb-2">{title}</p>
      {/* <hr className='text-black'></hr> */}
      <p className="text-base font-bold text-center p-2 border-t-[1px]">View Course</p>
    </Link>
  );
}
